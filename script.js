...
function switchView(viewName) {
  ...
  // Opuszczenie miksera zamyka pełnoekranowy radar
  if (viewName !== 'mixer') closeRadarOverlay();
...
function setMixerTab(tabName) {
  ...
  // Zmiana zakładki na inną niż obiekty zamyka pełnoekranowy radar
  if (tabName !== 'objects') closeRadarOverlay();
}
...
function setupEventHandlers() {
  ...
  // === Pełnoekranowy radar 3D ===
  document.getElementById('btnOpenRadar')?.addEventListener('click', openRadarOverlay);
  document.getElementById('btnCloseRadar')?.addEventListener('click', closeRadarOverlay);
  // Dotknięcie przyciemnionego tła (poza kartą radaru i nagłówkiem) zamyka nakładkę
  document.getElementById('radarOverlay')?.addEventListener('click', (e) => {
    if (e.target.id === 'radarOverlay' || e.target.classList.contains('radar-stage')) {
      closeRadarOverlay();
    }
  });
