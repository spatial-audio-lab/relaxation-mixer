...
function renderMeditationList() {
  ...
  container.innerHTML = CONFIG.sessions.map(session => {
    const coverStyle = session.cover ? ` style="background-image:url('${session.cover}')"` : '';
    return `
    <div class="library-card" data-id="${session.id}">
      <div class="library-card-cover"${coverStyle}>${session.cover ? '' : session.icon}</div>
