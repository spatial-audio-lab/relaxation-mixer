...
function switchView(viewName) {
  ...
  if (viewName === 'player') {
    homeView?.classList.add('hidden-layer');
    playerView?.classList.remove('hidden-layer');
  } else {
    playerView?.classList.add('hidden-layer');
    homeView?.classList.remove('hidden-layer');
  }
...
function openMixer() {
  state.mixerOpen = true;
  document.getElementById('mixerDrawer')?.classList.add('open');
  document.getElementById('mixerBackdrop')?.classList.add('visible');

  // Canvas potrzebuje realnych wymiarów — dociągamy je gdy panel staje się widoczny
  if (state.mixerTab === 'objects') {
...
function closeMixer() {
  ...
  document.getElementById('mixerDrawer')?.classList.remove('open');
  document.getElementById('mixerBackdrop')?.classList.remove('visible');
}
...
function renderMeditationList() {
  ...
  container.innerHTML = CONFIG.sessions.map((session, i) => `
    <div class="library-card" data-id="${session.id}" style="--i:${i}">
      <div class="library-card-cover cover-art" style="${coverStyleAttr(session)}">
        ${coverContent(session)}
        <div class="playing-badge" aria-hidden="true">${EQ_BARS_HTML}</div>
      </div>
      <div class="library-card-title">${session.name}</div>
      <div class="library-card-meta">${session.duration ? session.duration + ' • ' : ''}${session.description}</div>
    </div>
  `).join('');
}
...
function renderSpaceList() {
  ...
  container.innerHTML = CONFIG.scenes.map((scene, i) => {
    ...
    return `
    <div class="item-card" data-id="${scene.id}" style="--i:${i}">
      ...
      <div class="item-status">${EQ_BARS_HTML}</div>
    </div>
  `;
  }).join('');
}
...
function renderSoundsList() {
  ...
  container.innerHTML = CONFIG.objects.map((obj, i) => {
    const objState = state.sounds.objects[obj.id];
    const vol = Math.round((objState?.volume ?? 0.7) * 100);
    return `
    <div class="item-card sound-card" data-id="${obj.id}" style="--i:${i}">
      <div class="item-icon cover-art" style="${coverStyleAttr(obj)}">${coverContent(obj)}</div>
      <div class="item-info">
        <div class="item-name">${obj.name}</div>
        <div class="item-desc">Przytrzymaj → pozycja 3D</div>
        <div class="item-inline-volume">
          <span class="item-inline-volume-icon">🔊</span>
          <input type="range" class="inline-volume-slider" min="0" max="100" value="${vol}" aria-label="Głośność: ${obj.name}">
        </div>
      </div>
      <div class="item-status">${EQ_BARS_HTML}</div>
    </div>
...
function renderSpacesRow() {
  ...
  container.innerHTML = CONFIG.scenes.map((scene, i) => `
    <div class="space-chip" data-id="${scene.id}" style="--i:${i}">
...
function renderContinueSection() {
  ...
  row.innerHTML = `
    <div class="continue-card" data-id="${session.id}">
...
function syncMeditationUI() {
  ...
  // Siatka biblioteki
  document.querySelectorAll('#meditationGrid .library-card').forEach(card => {
...
function syncMiniPlayer() {
  ...
  document.getElementById('home-view')?.classList.toggle('has-mini-player', hasSelection);
...
function syncSpaceUI() {
  ...
  document.querySelectorAll('#spacesRow .space-chip').forEach(chip => {
    chip.classList.toggle('active', state.space.active === chip.dataset.id);
  });
}
...
function syncSoundsUI() {
  ...
  document.querySelectorAll('#soundsList .sound-card').forEach(card => {
    ...
    // Inline suwak głośności (widoczny tylko dla włączonych obiektów)
    const slider = card.querySelector('.inline-volume-slider');
    if (slider && objState && document.activeElement !== slider) {
      slider.value = Math.round(objState.volume * 100);
    }
  });
}
...
function setupEventHandlers() {
  ...
  document.getElementById('btnCloseMixer')?.addEventListener('click', closeMixer);
  document.getElementById('mixerBackdrop')?.addEventListener('click', closeMixer);
  ...
  // Siatka biblioteki (Home)
  document.getElementById('meditationGrid')?.addEventListener('click', async (e) => {
  ...
  // Rząd "Przestrzenie" (Home) — szybki start ambientu
  document.getElementById('spacesRow')?.addEventListener('click', async (e) => {
  ...
  // Inline suwak głośności — Obiekty 3D (nie uruchamia toggle ani long-press)
  document.getElementById('soundsList')?.addEventListener('input', (e) => {
    const slider = e.target.closest('.inline-volume-slider');
    if (!slider) return;
    const card = slider.closest('.sound-card');
    if (!card) return;
    updateObjectVolume(card.dataset.id, slider.value / 100);
  });
  ...
  function endSoundPress(e) {
    if (e.target.closest('.item-inline-volume')) return;
