...
function renderSoundsList() {
  ...
  container.innerHTML = CONFIG.objects.map((obj, i) => {
    ...
    return `
    <div class="item-card sound-card" data-id="${obj.id}" style="--i:${i}" tabindex="0" role="switch" aria-checked="false" aria-label="Dźwięk 3D: ${obj.name}">
      <div class="item-icon cover-art" style="${coverStyleAttr(obj)}">${coverContent(obj)}</div>
      <div class="item-info">
        <div class="item-name">${obj.name}</div>
        <div class="item-desc">Przytrzymaj lub ⚙ → pozycja 3D</div>
        <div class="item-inline-volume">
          <span class="item-inline-volume-icon">🔊</span>
...
function syncSoundsUI() {
  ...
  document.querySelectorAll('#soundsList .sound-card').forEach(card => {
    ...
    card.classList.toggle('loading', objState?.isLoading || false);
    card.setAttribute('aria-checked', String(enabled));
    card.setAttribute('aria-busy', String(objState?.isLoading || false));
...
function setupEventHandlers() {
  ...
  // Przycisk ⚙ — klawiaturowa/dotykowa alternatywa dla long-press (otwiera panel pozycji 3D)
  document.getElementById('soundsList')?.addEventListener('click', (e) => {
    const btn = e.target.closest('.btn-3d-position');
    if (!btn) return;
    e.stopPropagation();
    selectObjectFor3DControl(btn.dataset.id);
  });

  function startSoundPress(e) {
    if (e.target.closest('.item-inline-volume') || e.target.closest('.btn-3d-position')) return; // suwak głośności / przycisk pozycji mają własną obsługę
  ...
  function endSoundPress(e) {
    if (e.target.closest('.item-inline-volume') || e.target.closest('.btn-3d-position')) return;
